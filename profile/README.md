...
- **현재 한계**: 파이프라인이 타 언론사 기사를 직접 크롤링해 수집하고 그 내용을 인용해 초안을 생성하는 구조인데, 각 언론사의 robots.txt·이용약관 등 크롤링 정책과 기사 저작권(인용 허용 범위) 문제를 설계 단계에서 고려하지 못했습니다. 실험 목적에서는 동작하지만, 실제 서비스로 운영하기에는 법적 리스크가 있습니다.
- **향후 계획**: 정식 라이선스 기반 뉴스 데이터 확보 경로(예: 한국언론진흥재단 뉴스 저작권 계약)를 검토하고, 생성 단계에서 인용 비율 제한과 출처 표기를 검증하는 로직을 추가할 계획입니다.

### 3. 에이전트 분리에 따른 토큰 사용량 증가

- **현재 한계**: 에이전트를 역할별로 분리해 각자 맡은 작업에만 집중하도록 설계했지만, 의도와 달리 토큰 사용량이 기하급수적으로 늘었습니다. 각 에이전트가 전체 맥락을 모르는 상태가 되자 할루시네이션이 급격히 증가했고, 이를 줄이기 위해 프롬프트에 맥락 설명과 few-shot 예시를 계속 추가하다 보니 인풋 토큰 소모가 크게 늘었습니다. 늘어난 토큰은 그대로 추론 시간 증가로 이어져 속도 저하 문제까지 따라왔습니다.
- **향후 계획**: 단계별로 필요한 최소 맥락만 선별해 전달하는 공유 상태(state) 요약 구조와 프롬프트 캐싱을 검토하고, 역할이 인접한 에이전트는 통합하는 방안도 함께 재검토할 예정입니다.

### 4. 오픈웨이트 모델 자체 구동의 운영 부담

- **현재 한계**: Qwen3-8B를 외부 API가 아닌 자체 장비에서 직접 구동하다 보니 GPU 메모리 등 리소스 관리, 장시간 추론 시 장비 발열, 느린 생성 속도 등 운영 전반의 관리 부담이 컸습니다. 파이프라인 전체 실행 시간도 모델 추론 속도에 크게 좌우되었습니다.
- **향후 계획**: 양자화(Quantization)나 vLLM 등 추론 최적화 도입, 또는 비용·품질을 비교해 관리형 API와 병행 운영하는 방안을 검토할 예정입니다.
