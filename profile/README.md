...
- **현재 한계**: 제목에 가중치를 많이 두고 있는 방식이라 의미가 다르더라고 ~과제를 해결하기 위해, ~했다. 등의 제목의 형태가 비슷하면 같은 이슈로 묶거나, 하나의 이슈를 여러 개의 이슈로 과분할 하는 문제가 있다. ------------수정중

- **향후 계획**: 이를 해결하기 위해  엔티티 방식을 도입해보려 합니다. 기사에서 추출한 핵심 개채명(인명, 기관명, 지명 등 고유명사)인 NER을 기반으로 최소 1개 이상 겹치지 않는 기사 쌍은 유사도 점수가 아무리 높더라도 동일 군집으로 묶이지 않도록 필터링을 추가하는 방식을 도입해 보려 합니다. ------------수정중
