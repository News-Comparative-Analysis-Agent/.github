...
<br>
##  클러스터링 방법론 
본 프로젝트는 실시간으로 쏟아지는 뉴스 스트림 속에서 독립적인 '이벤트(이슈)'를 정밀하게 탐지하기 위해 **어휘적(Lexical) 정보와 의미적(Semantic) 정보를 융합한 하이브리드 클러스터링 모델을 설계하여 적용했습니다.
...
이를 극복하기 위해, 뉴스 이벤트 탐지(New Event Detection) 관련 최신 연구들을 참고하여 **사전 파인튜닝 없이도 실시간 고성능을 보장하는 "TF-IDF + SBERT 가중합(Weighted Sum) 모델"**을 고안했습니다.
---
###  제안하는 하이브리드 군집화 알고리즘
